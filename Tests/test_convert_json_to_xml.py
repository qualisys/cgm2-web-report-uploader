from qtmWebGaitReport.convert_report_json_to_regression_test_xml import get_xml_string_from_report_json
...
def load_previous_session_data_xml():
    path_to_previous_session_data = os.path.join(
        "TestFiles", "ClinicalGaitExample", "session_data.xml")
...
def test_check_that_xml_string_has_not_changed(previous_report_xml_string):
    report_xml_string = get_xml_string_from_report_json(report_json_data)
    assert report_xml_string == previous_report_xml_string, "xml string has changed"

    # # save xml to file
    # xml_filename = "session_data.xml"
