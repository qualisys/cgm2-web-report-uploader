...
from qtmWebGaitReport import c3dValidation, qtools, segmentCS
...
class SkeletonViewer:
    def __init__(self, workingDirectory):
        ...
        c3dValObj = c3dValidation.c3dValidation(
            workingDirectory
        )  # create list of signals. Make sure that missing signal in one of trial will not break it
        ...
        for filename in self.fileNames:
            ...
            if "1" in measurementName:  # temporaty
                ...
                root = {
                    ...
                    "frames": frames,
                    "force": {"lengthUnit": "mm", "forceUnit": "F", "plates": self.calculateForcePlateInfo(acq)},
                }
                    ...
                    if qtools.isPointExist(acq, sigName + "O"):
                        ...
                        for frame in range(noFrames):
                            sig[sigName] = np.vstack(
                                (sig[sigName], self.combineCardanSeqPerFrame(acq, sigName.split(), frame))
                            )
        return sig
    ...
    def segments(self, acq):
        segments = [
            {"name": "Pelvis", "length": self.getSegmentLength(acq, "RASI", "LASI")},
            {"name": "RThigh", "length": self.getSegmentLength(acq, "RFEP", "RFEO")},
            {"name": "RLeg", "length": self.getSegmentLength(acq, "RFEO", "RTIO")},
            {"name": "RFoot", "length": self.getSegmentLength(acq, "RTIO", "RTOE")},
            {"name": "LThigh", "length": self.getSegmentLength(acq, "LFEP", "LFEO")},
            {"name": "LLeg", "length": self.getSegmentLength(acq, "LFEO", "LTIO")},
            {"name": "LFoot", "length": self.getSegmentLength(acq, "LTIO", "LTOE")},
        ]

        if qtools.isPointExist(acq, "LSHO") and qtools.isPointExist(acq, "RSHO"):
            segments.append({"name": "Thorax", "length": self.getSegmentLength(acq, "LSHO", "RSHO")})
        if qtools.isPointExist(acq, "RSHO") and qtools.isPointExist(acq, "RELB"):
            segments.append({"name": "RArm", "length": self.getSegmentLength(acq, "RSHO", "RELB")})
        if qtools.isPointExist(acq, "RELB") and qtools.isPointExist(acq, "RWRB"):
            segments.append({"name": "RForearm", "length": self.getSegmentLength(acq, "RELB", "RWRB")})
        if qtools.isPointExist(acq, "RWRB") and qtools.isPointExist(acq, "RFIN"):
            segments.append({"name": "RHand", "length": self.getSegmentLength(acq, "RWRB", "RFIN")})
        if qtools.isPointExist(acq, "LSHO") and qtools.isPointExist(acq, "LELB"):
            segments.append({"name": "LArm", "length": self.getSegmentLength(acq, "LSHO", "LELB")})
        if qtools.isPointExist(acq, "LELB") and qtools.isPointExist(acq, "LWRB"):
            segments.append({"name": "LForearm", "length": self.getSegmentLength(acq, "LELB", "LWRB")})
        if qtools.isPointExist(acq, "LWRB") and qtools.isPointExist(acq, "LFIN"):
            segments.append({"name": "LHand", "length": self.getSegmentLength(acq, "LWRB", "LFIN")})
        if qtools.isPointExist(acq, "LFHD") and qtools.isPointExist(acq, "RFHD"):
            segments.append({"name": "Head", "length": self.getSegmentLength(acq, "RFHD", "LFHD") * 2})
    ...
    def calculateForcePlateInfo(self, acq):
        ...
        for fp in range(int(noForcePlates[0])):
            ...
            origin[fp] = (
                (dimensionX[fp][0] + dimensionX[fp][1]) / 2,
                (dimensionY[fp][0] + dimensionY[fp][1]) / 2,
                (dimensionZ[fp][0] + dimensionZ[fp][1]) / 2,
            )
            i = i + 4
            j = j + 4

            plates.append({"id": fp + 1, "name": "", "length": sizeX[fp], "width": sizeY[fp], "origin": origin[fp]})
        return plates
